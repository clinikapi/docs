...
Every Consent must carry a `policyRule` or a `policyUri` — FHIR invariant
ppc-1 rejects one that has neither. Pass your own policy reference from the
proxy if `hipaa-auth` isn't the right identifier for your organization.
