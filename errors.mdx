...
**Test keys have no monthly ceiling on a paid plan.** A `clk_test_*` key writes
to the sandbox datastore, and none of that traffic is counted or billed — it is
still bounded by the per-minute rate limit, but not by the monthly pool. On the
Sandbox plan there is no live environment, so those test requests are exactly
what consume the 1,000.
